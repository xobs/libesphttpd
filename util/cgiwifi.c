...
CgiStatus ICACHE_FLASH_ATTR cgiWiFiScan(HttpdConnData *connData)
{
	...
	if (!cgiWifiAps.scanInProgress && pos != 0) {
		...
		if (pos - 1 < cgiWifiAps.noAps) {
			len =
			    sprintf(buff,
				    "{\"essid\": \"%s\", \"bssid\": \"" MACSTR "\", \"rssi\": \"%d\", \"enc\": \"%d\", "
									       "\"channel\": \"%d\"}%s\n",
				    ...
				    cgiWifiAps.apData[pos - 1]->channel, (pos - 1 == cgiWifiAps.noAps - 1) ? "" : ",");
...
CgiStatus ICACHE_FLASH_ATTR tplWlan(HttpdConnData *connData, char *token, void **arg)
{
	...
	if (strcmp(token, "WiFiMode") == 0) {
		...
		x = wifi_get_opmode();
		if (x == 2) {
			strcpy(buff, "<b>Can't scan in this mode.</b> Click <a href=\"setmode.cgi?mode=3\">here</a> to "
				     "go to STA+AP mode.");
		} else {
