...
int ICACHE_FLASH_ATTR cgiWebsockBroadcast(HttpdInstance *pInstance, const char *resource, char *data, int len,
					  int flags)
{
	Websock *lw = llStart;
	int ret = 0;
...
CgiStatus ICACHE_FLASH_ATTR cgiWebSocketRecv(HttpdInstance *pInstance, HttpdConnData *connData, char *data, int len)
{
	...
	for (i = 0; i < len; i++) {
		...
		if (ws->priv->wsStatus == ST_FLAGS) {
			...
			if (((ws->priv->fr.len8 & 127) == 126 && ws->priv->wsStatus == ST_LEN2) ||
			    ws->priv->wsStatus == ST_LEN8) {
				ws->priv->wsStatus = (ws->priv->fr.len8 & IS_MASKED) ? ST_MASK1 : ST_PAYLOAD;
			} else {
				ws->priv->wsStatus++;
			}
		...
		if (ws->priv->wsStatus == ST_PAYLOAD) {
			...
			sl = len - i;
			ESP_LOGD(TAG, "Frame payload. wasHeaderByte %d fr.len %d sl %d cmd 0x%x", wasHeaderByte,
				 (int)ws->priv->fr.len, (int)sl, ws->priv->fr.flags);
			...
			if ((ws->priv->fr.flags & OPCODE_MASK) == OPCODE_PING) {
				...
				if (!ws->priv->closedHere) {
					...
					cgiWebsocketClose(pInstance, ws,
							  ((data[i] << 8) & 0xff00) + (data[i + 1] & 0xff));
				}
				r = HTTPD_CGI_DONE;
				break;
			} else {
